...
async function testCoupons() {
  try {
    ...
    const existingTest = await db.select().from(coupons).where(eq(coupons.shortcode, 'TEST10')).limit(1);
    ...
    const testCouponData = await db.select().from(coupons).where(eq(coupons.shortcode, 'TEST10')).limit(1);
